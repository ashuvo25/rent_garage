...
public class cencleController {
    ...
    public void initialize() {
        ...
        try {
            ...
            // Debugging: Inspect each entry in the list
            for (Map<String, Object> map : rent) {
                System.out.println("Inspecting entry: " + map);
                if ("run".equals(map.get("state")) && RentGaradge.user_id.equals(map.get("renter"))) {
                    renterdetails = map;
                    System.out.println("Matching entry found: " + map);
    ...
    private void openSignupWindow() {
        if (renterdetails == null) {
            System.out.println("No renter details found.");
            return; // Exit early if renterdetails is null
        }
